...
{
    internal class NotificationServiceImpl : UNUserNotificationCenterDelegate, INotificationService, IUNUserNotificationCenterDelegate
    {
        ...
        (
            token =>
            {
                onTokenRefresh?.Invoke(LocalNotificationCenter.Current, new FirebasePushNotificationTokenEventArgs(token));
            }
        );
        ...
        public static void Initialize(NSDictionary options, bool isFirebase, bool autoRegistration = true)
        {
            if (isFirebase)
            {
                ...
                if (autoRegistration)
                    LocalNotificationCenter.Current.RegisterForPushNotifications();
            }
            else
            {
